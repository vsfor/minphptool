...
class YiiFileHelper
{
    ...
    {
        $aliases = static::loadMimeAliases(__DIR__.'/mimeAliases.php');
    ...
    {
        if ($aliasesFile === null) {
            $aliasesFile = __DIR__.'/mimeAliases.php';
        }
    ...
    private static function matchBasename($baseName, $pattern, $firstWildcard, $flags)
    {
        if ($firstWildcard === false) {
            ...
            $n = YiiStringHelper::byteLength($pattern);
            if (YiiStringHelper::byteSubstr($pattern, 1, $n) === YiiStringHelper::byteSubstr($baseName, -$n, $n)) {
                return true;
            }
        }
        ...
        return YiiStringHelper::matchWildcard($pattern, $baseName, $matchOptions);
    }
    ...
    private static function matchPathname($path, $basePath, $pattern, $firstWildcard, $flags)
    {
        ...
        if (isset($pattern[0]) && $pattern[0] === '/') {
            $pattern = YiiStringHelper::byteSubstr($pattern, 1, YiiStringHelper::byteLength($pattern));
            if ($firstWildcard !== false && $firstWildcard !== 0) {
                $firstWildcard--;
            }
        }

        $namelen = YiiStringHelper::byteLength($path) - (empty($basePath) ? 0 : YiiStringHelper::byteLength($basePath) + 1);
        $name = YiiStringHelper::byteSubstr($path, -$namelen, $namelen);

        if ($firstWildcard !== 0) {
            if ($firstWildcard === false) {
                $firstWildcard = YiiStringHelper::byteLength($pattern);
            }
            ...
            $pattern = YiiStringHelper::byteSubstr($pattern, $firstWildcard, YiiStringHelper::byteLength($pattern));
            $name = YiiStringHelper::byteSubstr($name, $firstWildcard, $namelen);
        ...
        return YiiStringHelper::matchWildcard($pattern, $name, $matchOptions);
    }
    ...
    private static function parseExcludePattern($pattern, $caseSensitive)
    {
        ...
        if (YiiStringHelper::byteLength($pattern) && YiiStringHelper::byteSubstr($pattern, -1, 1) === '/') {
            $pattern = YiiStringHelper::byteSubstr($pattern, 0, -1);
        ...
        if ($pattern[0] === '*' && self::firstWildcardInPattern(YiiStringHelper::byteSubstr($pattern, 1, YiiStringHelper::byteLength($pattern))) === false) {
